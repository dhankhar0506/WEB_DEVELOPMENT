...
`main.jsx` connects **React with `index.html`**.
...
`main.jsx` / `index.js` → `main.jsx` is the entry point of a React application.

It creates the React root and renders the root `App` component into the HTML root element.
...
`package.json` is the main description/configuration file of your Node/React project.
...
`.env` is mainly used for **environment-specific configuration** — values that may be different in development, testing, and production.
...
| `context/`          | React Context-related code                 |
