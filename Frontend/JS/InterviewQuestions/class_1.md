...
## differnce between normal functon and arrow Function?
...
- A normal function has its own this, and the value of this depends on how the function is called.

>No. You cannot use the new keyword with an arrow function. Arrow functions do not have their own this or prototype, so they cannot be used as constructors. 

## new keyword?
->The new keyword is used to create a new object from a constructor function. It creates a new object, sets this to that object, links the object's prototype to the constructor's prototype, and returns the object.
...
## is Arrow Function is hoisted?
=> Arrow functions don't have special hoisting behavior. Their hoisting behavior depends on how they are declared. When assigned to let or const, they are hoisted but remain in the Temporal Dead Zone until initialization."
...
So you cannot use an arrow function before its declaration when it is assigned to const.
...
-> The Event Loop coordinates between the Call Stack and the queues. Microtasks are given priority and are processed before the next task/macrotask.
...
## type of popup boxes ?
1. alert("Hello!");
2. const result = confirm("Are you sure?");
3. const name = prompt("Enter your name:");
>Yes, exactly. alert(), confirm(), and prompt() are Web APIs provided by the browser, not built into the JavaScript language itself.
...
## JSON.stringfy()
-> Converts a JavaScript object/value into a JSON string.

## differnce between call , apply , bind
...
> Arrays and Strings
    ...
    [1] + 1         // "11"
    [1, 2] + 1      // "1,21"
    -> Arrays are converted to strings in these operations:
...
1. [localStorage] localStorage persists even after closing the tab or browser. It remains until you manually remove it or the site/browser clears it.
...
2. [sessionStorage] sessionStorage persists while the browser tab remains open. When that tab is closed, its data is normally cleared.
...
## Named Export vs Default Export in JavaScript
> Named Export: 
A named export allows us to export multiple variables, functions, classes, etc. from a module using their specific names.
    ...
    we import them using cury brackets

> Default Export
-> A default export is used to export one main value from a module. A module can have only one default export.
